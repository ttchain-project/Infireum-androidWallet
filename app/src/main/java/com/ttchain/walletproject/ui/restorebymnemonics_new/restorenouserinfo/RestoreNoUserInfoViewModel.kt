...
class RestoreNoUserInfoViewModel(
    private val context: Context,
    private val baseMainModel: BaseMainModel,
    private val restoreRepository: RestoreRepository
) : BaseViewModel() {

    //    val responseUserIdentitySubject = BehaviorSubject.create<ResponseUserIdentity>()
    ...
    private fun performRestoreIdentity() {
        viewModelLaunch {
            ...
            val createSelectionDataStatus = if (firstStatus) {
                restoreRepository.createCoinSelectionDataList()
                true
            } else {
                false
            }
            val initAssetStatus = if (createSelectionDataStatus) {
